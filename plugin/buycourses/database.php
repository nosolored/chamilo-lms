...
if (false === $sm->tablesExist(BuyCoursesPlugin::TABLE_COUPON)) {
    ...
    $couponTable->addColumn('active', Types::INTEGER);
...
$countryList = Database::select(
    '*',
    $currencyTable
);
$countryFieldValues = [];
foreach ($countryList as $item) {
    $countryFieldValues[] = html_entity_decode($item['country_name']);
}
$countryFieldValues = implode(";", $countryFieldValues);
...
$values = [
    ...
    'field_type' => 4,
    'variable' => 'country',
    'field_options' => $countryFieldValues,
    'visible_to_self' => 1,
    'visible_to_others' => 1,
    'changeable' => 1,
    'filter' => 1,
